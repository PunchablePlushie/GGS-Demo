...
#	it is (horizontal or vertical). Use -1 for this key if the default keybind
#	not a stick.
...
func main(value: Dictionary) -> void:
	...
	var prev_event: InputEvent = Utils.array_find_type(action_list, "InputEventJoypadButton") as InputEventJoypadButton
	
	if prev_event == null:
		prev_event = Utils.array_find_type(action_list, "InputEventJoyMotion") as InputEventJoypadMotion
